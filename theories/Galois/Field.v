...
(** Bridge from our [CommRing] record to Stdlib's [ring_theory], so we can
    use the [Add Ring] machinery (and the [ring] tactic) on any [CommRing].
    The signature lines up with what the Stdlib [Ring] plugin expects:
    parameters are zero, one, add, mul, sub, neg, eq.

    Most of the projections come straight from the [CommRing] axioms.  The
    only nontrivial repackaging is right-distributivity, which we obtain
    from left-distributivity plus commutativity of multiplication. *)
From Stdlib Require Import Ring.

Lemma cr_ring_theory {R : Type} (cr : CommRing R) :
    ring_theory cr.(cr_zero) cr.(cr_one)
                cr.(cr_add) cr.(cr_mul) (cr_sub cr) cr.(cr_neg) eq.
Proof.
  constructor.
  - (* Radd_0_l : 0 + x = x *)
    intro x. rewrite cr_add_comm. apply cr_add_zero.
  - (* Radd_comm *)
    intros x y. apply cr_add_comm.
  - (* Radd_assoc : x + (y + z) = (x + y) + z *)
    intros x y z. apply cr_add_assoc.
  - (* Rmul_1_l : 1 * x = x *)
    intro x. rewrite cr_mul_comm. apply cr_mul_one.
  - (* Rmul_comm *)
    intros x y. apply cr_mul_comm.
  - (* Rmul_assoc *)
    intros x y z. apply cr_mul_assoc.
  - (* Rdistr_l : (x + y) * z = x * z + y * z *)
    intros x y z.
    rewrite (cr_mul_comm cr (cr.(cr_add) x y) z).
    rewrite cr_distrib.
    rewrite (cr_mul_comm cr z x), (cr_mul_comm cr z y).
    reflexivity.
  - (* Rsub_def : x - y = x + -y *)
    intros x y. unfold cr_sub. reflexivity.
  - (* Ropp_def : x + -x = 0 *)
    intro x. apply cr_add_neg.
Qed.

Lemma cr_add_zero_l {R : Type} (cr : CommRing R) (a : R) :
    cr_add cr cr.(cr_zero) a = a.
Proof.
  rewrite cr_add_comm. apply cr_add_zero.
Qed.

Lemma cr_add_neg_l {R : Type} (cr : CommRing R) (a : R) :
    cr_add cr (cr_neg cr a) a = cr_zero cr.
Proof.
  rewrite cr_add_comm. apply cr_add_neg.
Qed.

Lemma cr_mul_zero_r {R : Type} (cr : CommRing R) (a : R) :
    cr_mul cr a cr.(cr_zero) = cr_zero cr.
Proof.
  set (t := cr_mul cr a cr.(cr_zero)).
  assert (Hd : t = cr_add cr t t).
  { unfold t.
    assert (Hdist := cr_distrib cr a cr.(cr_zero) cr.(cr_zero)).
    rewrite cr_add_zero in Hdist.
    exact Hdist. }
  assert (Hstep_a : cr_add cr (cr_add cr t t) (cr_neg cr t) = cr_zero cr).
  { rewrite <- Hd. apply cr_add_neg. }
  assert (Hstep_b : cr_add cr (cr_add cr t t) (cr_neg cr t) = t).
...
Lemma cr_mul_zero_l {R : Type} (cr : CommRing R) (a : R) :
    cr_mul cr cr.(cr_zero) a = cr_zero cr.
Proof.
  rewrite cr_mul_comm. apply cr_mul_zero_r.
Qed.

Lemma cr_neg_zero {R : Type} (cr : CommRing R) :
    cr_neg cr cr.(cr_zero) = cr_zero cr.
Proof.
  assert (H := cr_add_neg cr cr.(cr_zero)).
  rewrite cr_add_zero_l in H.
  exact H.
Qed.
