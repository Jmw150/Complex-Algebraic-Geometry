...
(* CAG zero-dependent Axiom mat_transpose_add theories/Lie/Symplectic.v:36 BEGIN
Axiom mat_transpose_add :
  forall {F : Type} (fld : Field F) (A B : Matrix F),
   ...
   CAG zero-dependent Axiom mat_transpose_add theories/Lie/Symplectic.v:36 END *)

(* CAG zero-dependent Axiom mat_transpose_neg theories/Lie/Symplectic.v:41 BEGIN
Axiom mat_transpose_neg :
  forall {F : Type} (fld : Field F) (A : Matrix F),
   ...
   CAG zero-dependent Axiom mat_transpose_neg theories/Lie/Symplectic.v:41 END *)

(* CAG zero-dependent Axiom mat_transpose_scale theories/Lie/Symplectic.v:45 BEGIN
Axiom mat_transpose_scale :
  forall {F : Type} (fld : Field F) (c : F) (A : Matrix F),
   ...
   CAG zero-dependent Axiom mat_transpose_scale theories/Lie/Symplectic.v:45 END *)

(* CAG zero-dependent Axiom mat_transpose_mul theories/Lie/Symplectic.v:49 BEGIN
Axiom mat_transpose_mul :
  forall {F : Type} (fld : Field F) (A B : Matrix F),
    mat_transpose fld (mat_mul fld A B) =
    mat_mul fld (mat_transpose fld B) (mat_transpose fld A).
   CAG zero-dependent Axiom mat_transpose_mul theories/Lie/Symplectic.v:49 END *)

(* CAG zero-dependent Axiom mat_transpose_zero theories/Lie/Symplectic.v:54 BEGIN
Axiom mat_transpose_zero :
  forall {F : Type} (fld : Field F) (n : nat),
    mat_transpose fld (mat_zero fld n) = mat_zero fld n.
   CAG zero-dependent Axiom mat_transpose_zero theories/Lie/Symplectic.v:54 END *)

(** [A,B]^T = [B^T, A^T]: transposing reverses the bracket. *)
(* CAG zero-dependent Lemma mat_transpose_bracket theories/Lie/Symplectic.v:59 BEGIN
...
Proof.
  ...
  reflexivity.
Qed.
   CAG zero-dependent Lemma mat_transpose_bracket theories/Lie/Symplectic.v:59 END *)

(* ================================================================== *)
(** * 2. Matrix algebra axioms                                        *)
(* ================================================================== *)

(* CAG zero-dependent Axiom mat_mul_add_distr_r theories/Lie/Symplectic.v:74 BEGIN
Axiom mat_mul_add_distr_r :
  forall {F : Type} (fld : Field F) (A B C : Matrix F),
   ...
   CAG zero-dependent Axiom mat_mul_add_distr_r theories/Lie/Symplectic.v:74 END *)

(* CAG zero-dependent Axiom mat_mul_add_distr_l theories/Lie/Symplectic.v:79 BEGIN
Axiom mat_mul_add_distr_l :
  forall {F : Type} (fld : Field F) (A B C : Matrix F),
    mat_mul fld A (mat_add fld B C) =
    mat_add fld (mat_mul fld A B) (mat_mul fld A C).
   CAG zero-dependent Axiom mat_mul_add_distr_l theories/Lie/Symplectic.v:79 END *)

(* CAG zero-dependent Axiom mat_mul_assoc_m theories/Lie/Symplectic.v:84 BEGIN
Axiom mat_mul_assoc_m :
  forall {F : Type} (fld : Field F) (A B C : Matrix F),
    mat_mul fld (mat_mul fld A B) C =
    mat_mul fld A (mat_mul fld B C).
   CAG zero-dependent Axiom mat_mul_assoc_m theories/Lie/Symplectic.v:84 END *)

(* CAG zero-dependent Axiom mat_mul_neg_l theories/Lie/Symplectic.v:89 BEGIN
Axiom mat_mul_neg_l :
  forall {F : Type} (fld : Field F) (A B : Matrix F),
    mat_mul fld (mat_neg fld A) B = mat_neg fld (mat_mul fld A B).
   CAG zero-dependent Axiom mat_mul_neg_l theories/Lie/Symplectic.v:89 END *)

(* CAG zero-dependent Axiom mat_mul_neg_r theories/Lie/Symplectic.v:93 BEGIN
Axiom mat_mul_neg_r :
  forall {F : Type} (fld : Field F) (A B : Matrix F),
    mat_mul fld A (mat_neg fld B) = mat_neg fld (mat_mul fld A B).
   CAG zero-dependent Axiom mat_mul_neg_r theories/Lie/Symplectic.v:93 END *)

(* CAG zero-dependent Axiom mat_mul_scale_l theories/Lie/Symplectic.v:97 BEGIN
Axiom mat_mul_scale_l :
  forall {F : Type} (fld : Field F) (c : F) (A B : Matrix F),
   ...
   CAG zero-dependent Axiom mat_mul_scale_l theories/Lie/Symplectic.v:97 END *)

(* CAG zero-dependent Axiom mat_mul_scale_r theories/Lie/Symplectic.v:101 BEGIN
Axiom mat_mul_scale_r :
  forall {F : Type} (fld : Field F) (c : F) (A B : Matrix F),
    mat_mul fld A (mat_scale fld c B) = mat_scale fld c (mat_mul fld A B).
   CAG zero-dependent Axiom mat_mul_scale_r theories/Lie/Symplectic.v:101 END *)

(* CAG zero-dependent Axiom mat_add_comm theories/Lie/Symplectic.v:105 BEGIN
Axiom mat_add_comm :
  forall {F : Type} (fld : Field F) (A B : Matrix F),
    mat_add fld A B = mat_add fld B A.
   CAG zero-dependent Axiom mat_add_comm theories/Lie/Symplectic.v:105 END *)

(* CAG zero-dependent Axiom mat_add_assoc_m theories/Lie/Symplectic.v:109 BEGIN
Axiom mat_add_assoc_m :
  forall {F : Type} (fld : Field F) (A B C : Matrix F),
    mat_add fld (mat_add fld A B) C =
    mat_add fld A (mat_add fld B C).
   CAG zero-dependent Axiom mat_add_assoc_m theories/Lie/Symplectic.v:109 END *)

(* CAG zero-dependent Axiom mat_add_neg theories/Lie/Symplectic.v:114 BEGIN
Axiom mat_add_neg :
  forall {F : Type} (fld : Field F) (A : Matrix F),
    mat_add fld A (mat_neg fld A) = mat_zero fld 0.
   CAG zero-dependent Axiom mat_add_neg theories/Lie/Symplectic.v:114 END *)

(* CAG zero-dependent Axiom mat_add_neg_l theories/Lie/Symplectic.v:118 BEGIN
Axiom mat_add_neg_l :
  forall {F : Type} (fld : Field F) (A : Matrix F),
    mat_add fld (mat_neg fld A) A = mat_zero fld 0.
   CAG zero-dependent Axiom mat_add_neg_l theories/Lie/Symplectic.v:118 END *)

(* CAG zero-dependent Axiom mat_add_zero_r theories/Lie/Symplectic.v:122 BEGIN
Axiom mat_add_zero_r :
  forall {F : Type} (fld : Field F) (A : Matrix F),
    mat_add fld A (mat_zero fld 0) = A.
   CAG zero-dependent Axiom mat_add_zero_r theories/Lie/Symplectic.v:122 END *)

(* CAG zero-dependent Axiom mat_add_zero_l theories/Lie/Symplectic.v:126 BEGIN
Axiom mat_add_zero_l :
  forall {F : Type} (fld : Field F) (A : Matrix F),
    mat_add fld (mat_zero fld 0) A = A.
   CAG zero-dependent Axiom mat_add_zero_l theories/Lie/Symplectic.v:126 END *)

(* CAG zero-dependent Axiom mat_neg_add theories/Lie/Symplectic.v:130 BEGIN
Axiom mat_neg_add :
  forall {F : Type} (fld : Field F) (A B : Matrix F),
    mat_neg fld (mat_add fld A B) =
    mat_add fld (mat_neg fld A) (mat_neg fld B).
   CAG zero-dependent Axiom mat_neg_add theories/Lie/Symplectic.v:130 END *)

(* CAG zero-dependent Axiom mat_neg_zero theories/Lie/Symplectic.v:135 BEGIN
Axiom mat_neg_zero :
  forall {F : Type} (fld : Field F) (n : nat),
    mat_neg fld (mat_zero fld n) = mat_zero fld 0.
   CAG zero-dependent Axiom mat_neg_zero theories/Lie/Symplectic.v:135 END *)

(* CAG zero-dependent Axiom mat_neg_neg theories/Lie/Symplectic.v:139 BEGIN
Axiom mat_neg_neg :
  forall {F : Type} (fld : Field F) (A : Matrix F),
    mat_neg fld (mat_neg fld A) = A.
   CAG zero-dependent Axiom mat_neg_neg theories/Lie/Symplectic.v:139 END *)

(* CAG zero-dependent Axiom mat_scale_add_mat theories/Lie/Symplectic.v:143 BEGIN
Axiom mat_scale_add_mat :
  forall {F : Type} (fld : Field F) (c : F) (A B : Matrix F),
    mat_scale fld c (mat_add fld A B) =
    mat_add fld (mat_scale fld c A) (mat_scale fld c B).
   CAG zero-dependent Axiom mat_scale_add_mat theories/Lie/Symplectic.v:143 END *)

(* CAG zero-dependent Axiom mat_scale_zero_mat theories/Lie/Symplectic.v:148 BEGIN
Axiom mat_scale_zero_mat :
  forall {F : Type} (fld : Field F) (c : F) (n : nat),
    mat_scale fld c (mat_zero fld n) = mat_zero fld 0.
   CAG zero-dependent Axiom mat_scale_zero_mat theories/Lie/Symplectic.v:148 END *)

(* CAG zero-dependent Axiom mat_mul_zero_l_n theories/Lie/Symplectic.v:152 BEGIN
Axiom mat_mul_zero_l_n :
  forall {F : Type} (fld : Field F) (n : nat) (A : Matrix F),
    mat_mul fld (mat_zero fld n) A = mat_zero fld 0.
   CAG zero-dependent Axiom mat_mul_zero_l_n theories/Lie/Symplectic.v:152 END *)

(* CAG zero-dependent Axiom mat_mul_zero_r_n theories/Lie/Symplectic.v:156 BEGIN
Axiom mat_mul_zero_r_n :
  forall {F : Type} (fld : Field F) (n : nat) (A : Matrix F),
    mat_mul fld A (mat_zero fld n) = mat_zero fld 0.
   CAG zero-dependent Axiom mat_mul_zero_r_n theories/Lie/Symplectic.v:156 END *)

(* ================================================================== *)
(** * 3. Standard symplectic form                                     *)
(* ================================================================== *)

(* CAG zero-dependent Axiom sp_form theories/Lie/Symplectic.v:164 BEGIN
Axiom sp_form : forall {F : Type} (fld : Field F) (l : nat), Matrix F.
   CAG zero-dependent Axiom sp_form theories/Lie/Symplectic.v:164 END *)

(** s^T = -s. *)
(* CAG zero-dependent Axiom sp_form_antisymm theories/Lie/Symplectic.v:167 BEGIN
Axiom sp_form_antisymm :
  forall {F : Type} (fld : Field F) (l : nat),
    mat_transpose fld (sp_form fld l) = mat_neg fld (sp_form fld l).
   CAG zero-dependent Axiom sp_form_antisymm theories/Lie/Symplectic.v:167 END *)

(* ================================================================== *)
(** * 4. Symplectic Lie algebra                                       *)
(* ================================================================== *)

(* CAG zero-dependent Definition IsSymplectic theories/Lie/Symplectic.v:177 BEGIN
Definition IsSymplectic {F : Type} (fld : Field F) (l : nat) (A : Matrix F) : Prop :=
  ...
  = mat_zero fld 0.
   CAG zero-dependent Definition IsSymplectic theories/Lie/Symplectic.v:177 END *)

(** From IsSymplectic A: A^T·s = -(s·A).
    Proof: X + Y = 0 → X = X + (Y + -Y) = (X+Y) + -Y = 0 + -Y = -Y. *)
(* CAG zero-dependent Lemma sp_cond_l theories/Lie/Symplectic.v:185 BEGIN
...
Qed.
   CAG zero-dependent Lemma sp_cond_l theories/Lie/Symplectic.v:185 END *)

(** From IsSymplectic A: s·A = -(A^T·s).
    Proof: X + Y = 0 → Y = 0 + Y = (-X+X) + Y = -X + (X+Y) = -X + 0 = -X. *)
(* CAG zero-dependent Lemma sp_cond_r theories/Lie/Symplectic.v:201 BEGIN
...
Qed.
   CAG zero-dependent Lemma sp_cond_r theories/Lie/Symplectic.v:201 END *)

(** sp(2l, F) is a Lie subalgebra of gl(2l, F). *)
(* CAG zero-dependent Lemma sp_is_subalgebra theories/Lie/Symplectic.v:216 BEGIN
...
Proof.
  ...
  - intros A B HA HB. unfold IsSymplectic in *.
    ...
    apply mat_add_neg.
Qed.
   CAG zero-dependent Lemma sp_is_subalgebra theories/Lie/Symplectic.v:216 END *)
...
(** The standard basis of sp(2l,F):
    ...
    Stated as a Conjecture pending the LieAlgebraF dimension function and
    explicit basis machinery. Reference: Humphreys §1.2, exercise 8. *)

(** dim sp(2l, F) = 2l^2 + l. *)
Theorem sp_dim : forall (l : nat),
    exists (d : nat), d = 2 * l * l + l.
Proof.
  intro l.
  exists (2 * l * l + l)%nat.
  reflexivity.
