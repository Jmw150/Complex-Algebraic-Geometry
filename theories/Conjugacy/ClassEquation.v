...
Proof.
  intros G sg G_dec G_list _ _.
  (* Both filters keep everything since their predicates are always true. *)
  induction G_list as [|x xs IH]; [reflexivity|].
  simpl. destruct (G_dec x x) as [_|Hne]; [|exfalso; apply Hne; reflexivity].
...
Conjecture pgroup_center_nontrivial :
...
(* quotient_by_center_cyclic_implies_abelian removed: false-as-stated.
   The hypothesis "∃ g, ∀ x, ∃ n, conj_class sg g (gpow x n)" is satisfied
   by ANY finite group (take g = e, n = |G|: x^|G| = e is conjugate to e
   by Lagrange). Counter: G = S_3 is non-abelian, but the hypothesis
   holds (n = 6 makes every x^n = e). Conclusion "G abelian" fails for S_3.
   The intended statement requires the hypothesis to talk about quotient
   G/Z(G), not arbitrary conjugacy. Was unused downstream. *)

Conjecture group_order_p_sq_abelian :
...
Conjecture group_order_p_sq_iso_cyclic_or_elementary_abelian :
  forall {G : Type} (sg : StrictGroup G) (p : nat)
         (p_prime : 2 <= p)
         (G_list : list G)
         (G_nodup : NoDup G_list)
...
Lemma A5_simple_via_conjugacy_classes : True. (* placeholder *)
Proof. exact I. Qed.
...
Conjecture conj_class_bounded_by_center_index :
  forall {G : Type} (sg : StrictGroup G)
...
Conjecture two_conj_classes_order_le_2 :
  forall {G : Type} (sg : StrictGroup G)
         (G_list reps : list G)
         (G_complete : forall x : G, In x G_list)
...
Conjecture not_union_of_conjugates :
  forall {G : Type} (sg : StrictGroup G)
         (G_list : list G)
         (G_complete : forall x : G, In x G_list)
         (G_nodup : NoDup G_list)
...
Conjecture transitive_has_fixpoint_free :
...
Conjecture pgroup_subgroups_all_orders :
  forall {G : Type} (sg : StrictGroup G) (p a : nat)
         (p_prime : 2 <= p)
         (G_list : list G)
         (G_complete : forall x : G, In x G_list)
...
Conjecture odd_order_not_conj_inv :
  forall {G : Type} (sg : StrictGroup G)
         (G_list : list G)
         (G_complete : forall x : G, In x G_list)
         (G_nodup : NoDup G_list)
